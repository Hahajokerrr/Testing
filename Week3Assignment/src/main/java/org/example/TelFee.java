...
public class TelFee {
    ...
    public String finalMessage() {
        String res = "";
        if((hour > 24 || hour < 0 || minute > 59 || minute < 0) && (duration <= 0)) {
            res += "Định dạng thời gian sai và thời lượng cuộc gọi phải là số dương";
        } else if(hour > 24 || hour < 0 || minute > 59 || minute < 0) {
            res += "Định dạng thời gian sai";
        } else if(duration <= 0) {
            res += "Thời lượng cuộc gọi phải là số dương";
        } else {
            res += finalFee();
        }
        return res;
